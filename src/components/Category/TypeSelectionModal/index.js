...
function TypeSelectionModal() {
  ...
  <label className={styles.Switch_Label} onClick={() => onChange(title)} htmlFor='switch' key={id}>{heading}</label>
  const ConcealedRadio = ( value, selected,disabled ) =>
  <input className={styles.Switch_Radio} type="radio" id="switch"  defaultChecked={selected === value} disabled={disabled}/>
  ...
  return(
    ...
    {difficulty.map((val) => (
        <div key={val.code}>
        <span>
          { ConcealedRadio(val.code,difficultyButton,val.code,val.disabled) }
    ...
    <>
      {isTypeSelectionModalOpen && (
        <div className={styles.backdrop} onClick={closeModalWindowHandler}>
          <div className={styles.modal_container} role="button" onClick={stopPropagation}>
            ...
            <div className = {styles.modecontainer}>
              ...
              <div className = 'modeToggle'>
                ...
                <span className={styles.switchmode} >{isTimed ? 'Timed' : 'Practice'}</span>
                </label>
              </div>
              <div className={styles.imgcontainer}>
            ...
            <br />
            <Button buttonStyle='btn--modal' className='styles.modalsubmit' type="submit" disabled={!difficultyButton || !quizMode} onclick={() =>
              dispatch(submitTypeSelectionModal(difficultyButton,quizMode,categoryId,categoryName))}>
              START
            </Button>
          </div>
        </div>
